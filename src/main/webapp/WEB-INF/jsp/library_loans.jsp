    ...
    <div class="loans-section">
      <table>
        ...
        <c:forEach var="loan" items="${loans}">
          <tr>
            <td>${loan.borrowID}</td>
            <td>${loan.bookID == 1 ? 'Atomic Habits' : 'The Alchemist'}</td>
            <td>${loan.userID == 1 ? 'John Doe' : 'Jane Smith'}</td>
            ...
            <td>${loan.fine}</td>
            <td class="${loan.returnDate == null ? 'status-overdue' : 'status-returned'}">${loan.returnDate == null ? 'Borrowed' : 'Returned'}</td>
          </tr>
        </c:forEach>
        </tbody>
      </table>
